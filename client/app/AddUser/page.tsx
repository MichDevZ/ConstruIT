...
const page = () => {
    ...
    const onSubmit = async (user: IUser) => {
        ...
        try {
            const data = await axios.post("http://localhost:5116/api/users", {
                rut: user.rut,
                name: user.name,
            ...
            if (axios.isAxiosError(error)) {
                if (error.response && error.response.status === 409) {
                    setvalidations("El rut ya se encuentra en uso")
...
<div className={`bg-red-500 text-white text-center mb-5 p-2 ${validations ? "" : "hidden"} `}>
  ...
  <div className="w-72 mx-auto">
    <div className="relative z-0 w-full mb-5 group">
        <input value={formData?.rut} maxLength={12}  onChange={(e) => setFormData({...formData, rut: formatRUT(e.target.value)})} type="text" name="floating_email" id="floating_email" className={`${!isValidRut(formData.rut) && validations ? "border-red-500": "border-gray-300" }  block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2  appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer`} placeholder=" " required />
    ...
    <div className="relative z-0 w-full mb-5 group">
        <input value={formData?.name} onChange={(e) => setFormData({...formData, name: e.target.value})} type="text" name="floating_email" id="floating_email" className={`${!/^[A-Za-zÁÉÍÓÚáéíóúñÑ\s]+$/.test(formData.name) && validations ? "border-red-500": "border-gray-300" }  block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2  appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer`} placeholder=" " required />
    ...
    <div className="relative z-0 w-full mb-5 group">
        <input value={formData?.lastName} onChange={(e) => setFormData({...formData, lastName: e.target.value})}  type="text" name="floating_email" id="floating_email" className={`${!/^[A-Za-zÁÉÍÓÚáéíóúñÑ\s]+$/.test(formData.name) && validations ? "border-red-500": "border-gray-300" }  block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2  appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer`} placeholder=" " required />
    ...
    <div className="relative z-0 w-full mb-5 group">
        <input value={formData?.dateOfBirth} onChange={(e) => setFormData({...formData, dateOfBirth: e.target.value})} type="date" name="floating_email" id="floating_email" className={`${!/^\d{4}-\d{2}-\d{2}$/.test(formData.dateOfBirth) && validations
        || Number(formData.dateOfBirth.split("-")[0]) > new Date().getFullYear() && validations ? "border-red-500": "border-gray-300" }  block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2  appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer`} placeholder=" " required />
    ...
    <div className="relative z-0 w-full mb-5 group">
        <input  value={formData?.email} onChange={(e) => setFormData({...formData, email: e.target.value})} type="email" name="floating_password" id="floating_password" className={`${!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email) && validations ? "border-red-500": "border-gray-300" }  block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2  appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer`} placeholder=" " required />
